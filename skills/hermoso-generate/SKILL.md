...
2. `hermoso auth login --url http://localhost:3000` — local dev needs **no token**; this just records the API base. (A `--token` is the seam for when hosted auth lands.)
...
5. **Report the served URL** (e.g. `http://localhost:3000/generated/…`), never a raw job id. If the user wants the file, `hermoso fetch <url> --out name.png`.
